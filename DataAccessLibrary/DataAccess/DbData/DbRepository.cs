...
namespace DataAccessLibrary.DataAccess.DbData
{
    public class DbRepository : IRepository
    {
        ...
        {
            List<Invite>? invites = await _db!.Invites.IgnoreAutoIncludes().Where(x => x.TargetUser.Username == username).ToListAsync();
            return invites!;
        }
        ...
        {
            Models.User? user = await GetUserAsync(receivingUsername)!;
            if (user is null)
            {
                return false;
            }
